...
@pytest.fixture
def results_dir():
    results_dir = [d for d in BASE_DIR.iterdir() if d.is_dir() and d.name == "results"]
    results_dir += [d for d in MAIN_DIR.iterdir() if d.is_dir() and d.name == "results"]
    return results_dir
...
def test_csv_files(results_dir):
    csv_files = [file for file in results_dir[0].iterdir() if file.glob("*.csv")]

    assert len(csv_files), (
        "В папке results не обнаружено csv-файлов " "с результатами работы парсера."
    )
