...
def test_pep_parse_pipeline():
    assert inspect.isclass(PepParsePipeline), "`PepParsePipeline` должен быть классом."
...
def test_pipeline_process_item():
    ...
    assert hasattr(got, "process_item"), (
        f"В классе `{got.__class__.__name__}` " "должен быть метод `process_item`."
    )
...
def test_pipeline_close_spider():
    ...
    assert hasattr(got, "close_spider"), (
        f"В классе `{got.__class__.__name__}` " "должен быть метод `close_spider`."
    )
